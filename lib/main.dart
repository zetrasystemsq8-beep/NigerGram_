...
void main() async {
  ...
  try {
    ...
    debugPrint(
      '✅ [STARTUP] Firestore offline persistence enabled - app will work offline',
    );
  } on TimeoutException catch (e) {
    debugPrint(
      '🔴 [STARTUP] Firebase initialization timed out: ${e.message}',
    );
  ...
  try {
    ...
    await Supabase.initialize(
      ...
      anonKey:
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNzbXd1aWhrYWZydWxtdnRpdWFtIiwiaWF0IjoxNzgwODI5NjYwLCJleHAiOjIwOTY0MDU2NjB9.e1PxmDW77ZhbonS-Z96SWA_sPyVGedzpZNZbJQz7pQo',
    ).timeout(
    ...
    debugPrint(
      '⚠️ [STARTUP] Supabase initialization timed out: ${e.message}',
    );
    debugPrint('⚠️ [STARTUP] Continuing anyway - Supabase is optional');
  } catch (e) {
...
class _NigerGramUpdateWrapper extends StatefulWidget {
  ...
  @override
  State<_NigerGramUpdateWrapper> createState() =>
      _NigerGramUpdateWrapperState();
}

class _NigerGramUpdateWrapperState
    extends State<_NigerGramUpdateWrapper> {
  String? _apkUrl;
  ...
  Future<void> _checkVersion() async {
    try {
      ...
      // No NigerGram row = don't block the app.
      if (result == null) {
        debugPrint(
      ...
      final minimumVersion =
          result['minimum_version']?.toString();

      final apkUrl =
          result['apk_url']?.toString();
      ...
      if (_isOutdated(
        ...
        if (apkUrl == null || apkUrl.isEmpty) {
          debugPrint(
            '⚠️ [UPDATE] App outdated but apk_url is missing.',
          );
          return;
        }
      ...
      // IMPORTANT:
      // Any update-check failure must NEVER prevent NigerGram
      // from working.
      debugPrint(
        '⚠️ [UPDATE] Version check failed. Continuing normally: $e',
      );
    }
  }
