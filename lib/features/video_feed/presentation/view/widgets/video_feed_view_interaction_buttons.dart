...
class _VideoFeedViewInteractionButtonsState extends State<VideoFeedViewInteractionButtons> {
      ...
      _videoSub = _firestore.collection('videos').doc(videoId).snapshots().listen((doc) {
        ...
        setState(() {
          ...
          if (AppAuth.isLoggedIn) {
            ...
            if (likedBy != null) {
              _isLiked = likedBy.contains(AppAuth.uid);
            }
            final savedBy = (data['savedBy'] as List<dynamic>?)?.cast<String>();
            if (savedBy != null) {
              _isSaved = savedBy.contains(AppAuth.uid);
            }
          }
        });
      }, onError: (e) {
  ...
  Future<void> _handleLike() async {
    ...
    try {
      final newStatus = await _repo.toggleLike(widget.videoId, AppAuth.uid);
  ...
  Future<void> _handleSave() async {
    ...
    try {
      await _repo.toggleSave(widget.videoId, AppAuth.uid);
