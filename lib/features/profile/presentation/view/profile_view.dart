...
// ╚══════════════════════════════════════════════════════════╝
//
// FEATURES NO OTHER PLATFORM HAS TOGETHER:
// ✅ Animated story ring with gradient glow
// ✅ Pinned videos showcase (top 3)
// ✅ Wallet balance in app bar (Monnify powered)
// ✅ Drafts manager with resume upload
// ✅ Q&A interactive section
// ✅ Achievement badges system
// ✅ Profile themes & customization
// ✅ Block & Report with confirmation
// ✅ Video download toggle per post
// ✅ Duet/Stitch privacy controls
...
// ─────────────────────────────────────────────────────────────────────────────
// CONSTANTS — NigerGram Design System
// ─────────────────────────────────────────────────────────────────────────────
...
class _ProfileViewState extends State<ProfileView>
  ...
  void initState() {
    ...
    // Story ring rotate animation
    _storyRotateController = AnimationController(
      duration: const Duration(seconds: 8),
      vsync: this,
    )..repeat();
    _storyRotateAnimation = Tween<double>(begin: 0, end: 2 * math.pi).animate(
      _storyRotateController,
    );
    
    _tabController = TabController(
      length: _isCurrentUser ? 6 : 5,
      vsync: this,
    );
  ...
  void _applyTheme() {
    switch (_profileTheme) {
      case 'gold':
        _accentColor = NGColors.accentGold;
        break;
      case 'blue':
        _accentColor = NGColors.accentBlue;
        break;
      case 'purple':
        _accentColor = NGColors.accentPurple;
        break;
      case 'green':
        _accentColor = NGColors.accentGreen;
        break;
      default:
        _accentColor = NGColors.accent;
    }
  }
  
  // ─────────────────────────────────────────────────────────────────────────
  // DATA LOADING — All parallel, all isolated
  // ─────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      debugPrint('Profile load fault: $e');
  ...
  Future<void> _loadUserData() async {
    final doc = await FirebaseFirestore.instance
        .collection('users').doc(_targetUserId).get();
  ...
  Future<void> _loadWalletBalance() async {
    ...
    final doc = await FirebaseFirestore.instance
        .collection('wallets').doc(_currentUid).get();
  ...
  Future<void> _checkStoryStatus() async {
    final snap = await FirebaseFirestore.instance
        .collection('users').doc(_targetUserId)
        .collection('stories')
  ...
  Future<void> _loadAchievements() async {
    final snap = await FirebaseFirestore.instance
        .collection('users').doc(_targetUserId)
        .collection('achievements').limit(10).get();
    if (mounted) setState(() =>
      _achievements = snap.docs.map((d) => d.data()).toList());
  }
  ...
  Future<void> _loadPinnedVideos() async {
    final snap = await FirebaseFirestore.instance
        ...
        .limit(3).get();
    if (mounted) setState(() =>
      _pinnedVideos = snap.docs.map((d) => d.data()).toList());
  }
  ...
  Future<void> _loadDrafts() async {
    final snap = await FirebaseFirestore.instance
        ...
        .orderBy('timestamp', descending: true).get();
    if (mounted) setState(() =>
      _draftVideos = snap.docs.map((d) => d.data()).toList());
  }
  ...
  Future<void> _loadQAItems() async {
    final snap = await FirebaseFirestore.instance
        ...
        .orderBy('timestamp', descending: true).limit(50).get();
    if (mounted) setState(() =>
      _qaItems = snap.docs.map((d) => d.data()).toList());
  }
  ...
  Future<void> _checkFollowStatus() async {
    ...
    final doc = await FirebaseFirestore.instance
        ...
        .collection('following').doc(_targetUserId).get();
  ...
  Future<void> _checkBlockStatus() async {
    ...
    final doc = await FirebaseFirestore.instance
        .collection('users').doc(_currentUid)
        .collection('blocked').doc(_targetUserId).get();
  ...
  Future<void> _toggleFollow() async {
    ...
    final followingRef = FirebaseFirestore.instance
        .collection('users').doc(_currentUid)
        .collection('following').doc(_targetUserId);
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(_isBlocked
            ? 'They will be able to see your profile and interact again.'
            : 'They won\'t be able to see your profile, message you, or interact with your content.',
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: Text(_isBlocked ? 'Unblock' : 'Block',
                  style: const TextStyle(color: NGColors.error))),
        ],
      ),
    );
    if (confirm != true || _isBlockLoading) return;
    ...
    try {
      if (_isBlocked) {
        await FirebaseFirestore.instance
            .collection('users').doc(_currentUid)
            .collection('blocked').doc(_targetUserId).delete();
      } else {
        await FirebaseFirestore.instance
            .collection('users').doc(_currentUid)
            .collection('blocked').doc(_targetUserId)
  ...
  Future<void> _pickAndUploadVideo(bool makePrivate) async {
    ...
    if (fileSize > maxSizeBytes) {
      if (mounted) _showSnack(
        'Video too large (${(fileSize / (1024 * 1024)).toStringAsFixed(1)}MB). Max 100MB.',
        isSuccess: false,
      );
      return;
    }
  ...
  Future<void> _deleteVideo(String videoId) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false),
              child: const Text('Cancel', style: TextStyle(color: NGColors.textMuted))),
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: const Text('Delete', style: TextStyle(color: NGColors.error))),
        ],
      ),
    );
    ...
    try {
      ...
      try {
        await _supabase.storage.from('nigergram-videos')
  ...
  Future<void> _togglePinVideo(String videoId, bool currentlyPinned) async {
    ...
    if (mounted) _showSnack(
      currentlyPinned ? 'Removed from pinned' : 'Pinned to profile!',
      isSuccess: true,
    );
  }
  ...
  Future<void> _updateAvatar() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(_currentUid)
  ...
  void _showEditSheet() {
    ...
    String selectedTheme = _profileTheme;
    bool saving = false;
                ...
                child: Column(
                  ...
                  children: [
                    Center(child: Container(width: 40, height: 4,
                        decoration: BoxDecoration(color: NGColors.divider, borderRadius: BorderRadius.circular(2)))),
                    const SizedBox(height: 20),
                    const Text('Edit Profile', style: TextStyle(color: NGColors.textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 20),
                    TextFormField(controller: nameCtrl, style: const TextStyle(color: NGColors.textPrimary, fontSize: 14),
                        decoration: _inputDeco('Display Name'),
                        validator: (v) => v!.trim().isEmpty ? 'Required' : null),
                    const SizedBox(height: 12),
                    TextFormField(controller: userCtrl, style: const TextStyle(color: NGColors.textPrimary, fontSize: 14),
                        decoration: _inputDeco('Username')),
                    const SizedBox(height: 12),
                    TextFormField(controller: bioCtrl, style: const TextStyle(color: NGColors.textPrimary, fontSize: 14),
                        decoration: _inputDeco('Bio'), maxLines: 3),
                    const SizedBox(height: 12),
                    TextFormField(controller: instaCtrl, style: const TextStyle(color: NGColors.textPrimary, fontSize: 14),
                        decoration: _inputDeco('Instagram URL')),
                    const SizedBox(height: 12),
                    TextFormField(controller: ytCtrl, style: const TextStyle(color: NGColors.textPrimary, fontSize: 14),
                    ...
                    Row(
                      ...
                      children: [
                        _themeOption(setSheet, 'default', 'Default', NGColors.accent, selectedTheme),
                        _themeOption(setSheet, 'gold', 'Gold', NGColors.accentGold, selectedTheme),
                        _themeOption(setSheet, 'blue', 'Blue', NGColors.accentBlue, selectedTheme),
                        _themeOption(setSheet, 'purple', 'Purple', NGColors.accentPurple, selectedTheme),
                        _themeOption(setSheet, 'green', 'Green', NGColors.accentGreen, selectedTheme),
                      ],
                    ),
                    ...
                    ElevatedButton(
                      onPressed: saving ? null : () async {
                        ...
                        try {
                          await _saveProfile(
                            ...
                            youtube: ytCtrl.text, theme: selectedTheme,
                          );
                      ...
                      child: saving
                          ? const SizedBox(width: 20, height: 20,
                              child: CircularProgressIndicator(color: NGColors.textPrimary, strokeWidth: 2))
                          : const Text('Save Changes',
                              style: TextStyle(color: NGColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 15)),
                    ),
  ...
  Widget _themeOption(StateSetter setSheet, String value, String label, Color color, String current) {
    final isSelected = current == value;
    return GestureDetector(
      onTap: () => setSheet(() => selectedTheme = value),
      child: Column(
        children: [
          Container(
            width: 36, height: 36,
            decoration: BoxDecoration(
              ...
              border: isSelected ? Border.all(color: NGColors.textPrimary, width: 3) : null,
            ),
            child: isSelected ? const Icon(Icons.check, color: NGColors.textPrimary, size: 18) : null,
          ),
          const SizedBox(height: 4),
          Text(label, style: TextStyle(color: isSelected ? color : NGColors.textMuted, fontSize: 10)),
        ],
      ),
    );
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          ListTile(
            leading: const CircleAvatar(backgroundColor: NGColors.surfaceLight,
                child: Icon(Icons.public_rounded, color: NGColors.success)),
          ...
          ListTile(
            leading: const CircleAvatar(backgroundColor: NGColors.surfaceLight,
                child: Icon(Icons.lock_outline_rounded, color: NGColors.accent)),
      ...
      builder: (ctx) => Column(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            onChanged: (val) async {
              setState(() => _allowDuet = val);
              await FirebaseFirestore.instance.collection('users').doc(_currentUid)
                  .update({'allowDuet': val});
            },
          ),
          SwitchListTile(
            ...
            activeColor: _accentColor,
            onChanged: (val) async {
              setState(() => _allowStitch = val);
              await FirebaseFirestore.instance.collection('users').doc(_currentUid)
                  .update({'allowStitch': val});
            },
          ),
          SwitchListTile(
      ...
      builder: (ctx) => Column(
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(video['isPinned'] == true ? 'Unpin from Profile' : 'Pin to Profile',
                style: const TextStyle(color: NGColors.textPrimary)),
            onTap: () {
              Navigator.pop(ctx);
              _togglePinVideo(video['videoId'], video['isPinned'] == true);
            },
          ),
          ListTile(
            ...
            onTap: () {
              Navigator.pop(ctx);
              Share.share('Watch this video on NigerGram!\nhttps://nigergram.app/video/${video['videoId']}');
            },
          ),
          ListTile(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Row(children: [
              Icon(Icons.analytics_rounded, color: _accentColor, size: 22),
              const SizedBox(width: 8),
              const Text('Profile Analytics', style: TextStyle(color: NGColors.textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
            ]),
            const SizedBox(height: 24),
  ...
  InputDecoration _inputDeco(String hint) => InputDecoration(
    labelText: hint,
    labelStyle: const TextStyle(color: NGColors.textMuted),
    filled: true,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // MAIN BUILD — THE ULTIMATE PROFILE
  // ─────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Scaffold(
        ...
        body: Center(child: CircularProgressIndicator(color: NGColors.accent)),
      );
    }
                ...
                return [
                  SliverAppBar(
                    ...
                    actions: [
                      if (_isCurrentUser) ...[
                        IconButton(
                          icon: Icon(Icons.account_balance_wallet_outlined, color: _accentColor),
                      ...
                      if (_isCurrentUser)
                        IconButton(
                          icon: Icon(Icons.analytics_outlined, color: _accentColor),
                      ...
                      PopupMenuButton<String>(
                        ...
                        itemBuilder: (ctx) => [
                          const PopupMenuItem(value: 'share',
                              child: Row(children: [Icon(Icons.share, color: NGColors.textPrimary, size: 18), SizedBox(width: 8), Text('Share Profile', style: TextStyle(color: NGColors.textPrimary))])),
                          if (!_isCurrentUser) ...[
                            PopupMenuItem(value: 'block',
                                child: Row(children: [Icon(Icons.block, color: NGColors.error, size: 18), SizedBox(width: 8), Text(_isBlocked ? 'Unblock' : 'Block', style: TextStyle(color: NGColors.error))])),
                            const PopupMenuItem(value: 'report',
                                child: Row(children: [Icon(Icons.flag, color: NGColors.warning, size: 18), SizedBox(width: 8), Text('Report', style: TextStyle(color: NGColors.warning))])),
                          ],
                        ],
                      ),
                    ],
                            ...
                            if (_isCurrentUser)
                              Positioned(
                                ...
                                child: Container(
                                  ...
                                  child: const Icon(Icons.camera_alt_rounded, color: NGColors.textPrimary, size: 16),
                                ),
                              ),
                          ],
                        ),
                  ...
                  SliverToBoxAdapter(
                    child: Padding(
                      padding: const EdgeInsets.symmetric(horizontal: NGSpacing.xl),
                      child: Column(
                        ...
                        children: [
                            ...
                            children: [
                                    ...
                                    return Transform.scale(
                                      ...
                                      child: CustomPaint(
                                        painter: _hasActiveStory ? _StoryRingPainter(
                                          colors: [_accentColor, NGColors.accentGold, NGColors.accentPurple],
                              ...
                              if (_isCurrentUser)
                                OutlinedButton(
                                  ...
                                  child: const Text('Edit Profile', style: TextStyle(color: NGColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w600)),
                                )
                              else ...[
                                ElevatedButton(
                                  onPressed: _toggleFollow,
                                  style: ElevatedButton.styleFrom(
                                    backgroundColor: _isFollowing ? NGColors.surfaceLight : _accentColor,
                                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                                  ...
                                  child: _isFollowLoading
                                      ...
                                      : Text(_isFollowing ? 'Following' : 'Follow',
                                          style: const TextStyle(color: NGColors.textPrimary, fontSize: 13, fontWeight: FontWeight.bold)),
                                ),
                              ],
                            ],
                          ),
                            ...
                            children: [
                              Flexible(
                                child: Text(
                                  ...
                                  style: const TextStyle(color: NGColors.textPrimary, fontSize: 20, fontWeight: FontWeight.bold),
                                  overflow: TextOverflow.ellipsis,
                                ),
                              ),
                              if (_userData?['isVerified'] == true) ...[
                                const SizedBox(width: 6),
                                const Icon(Icons.verified_rounded, color: NGColors.accentBlue, size: 18),
                              ],
                            ],
                          ),
                          ...
                          if (_userData?['bio'] != null && _userData!['bio'].toString().isNotEmpty)
                            Text(_userData!['bio'],
                                style: const TextStyle(color: NGColors.textSecondary, fontSize: 13, height: 1.4)),
                              ...
                              children: _achievements.map((a) => Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Text(a['title'] ?? '', style: const TextStyle(color: NGColors.textPrimary, fontSize: 11, fontWeight: FontWeight.w600)),
                                  ],
                                ),
                              )).toList(),
                            ),
                          ...
                          if (_pinnedVideos.isNotEmpty) ...[
                            Row(
                              children: [
                                const Icon(Icons.push_pin, color: NGColors.accentGold, size: 16),
                                const SizedBox(width: 4),
                                const Text('Pinned', style: TextStyle(color: NGColors.accentGold, fontSize: 13, fontWeight: FontWeight.bold)),
                              ],
                            ),
                            const SizedBox(height: 8),
                            SizedBox(
                                          ...
                                          Expanded(
                                            child: ClipRRect(
                                              ...
                                              child: v['thumbnailUrl'] != null && v['thumbnailUrl'].toString().isNotEmpty
                                                  ? CachedNetworkImage(imageUrl: v['thumbnailUrl'], fit: BoxFit.cover, width: double.infinity)
                                                  : const Center(child: Icon(Icons.play_circle_outline, color: NGColors.textMuted, size: 32)),
                                            ),
                                          ),
                                          Padding(
                                            padding: const EdgeInsets.all(6),
                                            child: Row(
                                              children: [
                                                ...
                                                Text('${v['viewCount'] ?? 0}', style: const TextStyle(color: NGColors.textPrimary, fontSize: 10)),
                                                const Spacer(),
                                                const Icon(Icons.push_pin, color: NGColors.accentGold, size: 10),
                                              ],
                                            ),
                                          ),
                                        ],
            ...
            SizedBox(
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    Icon(
                      tabName == 'private' ? Icons.lock_outline :
                      tabName == 'pinned' ? Icons.push_pin :
                      tabName == 'likes' ? Icons.favorite_border :
                      Icons.videocam_outlined,
                      color: NGColors.textMuted, size: 48,
                    ),
                    const SizedBox(height: 12),
                    Text(
                      tabName == 'private' ? 'No private videos' :
                      tabName == 'pinned' ? 'No pinned videos' :
                        ...
                        item['thumbnailUrl'] != null && item['thumbnailUrl'].toString().isNotEmpty
                            ? CachedNetworkImage(
                                ...
                                errorWidget: (_, __, ___) => const Center(
                                    child: Icon(Icons.play_circle_outline, color: NGColors.textMuted, size: 28)),
                              )
                            : const Center(child: Icon(Icons.play_circle_outline, color: NGColors.textMuted, size: 28)),
                        if (item['isPrivate'] == true)
                          Positioned(top: 4, right: 4,
                              child: Icon(Icons.lock, color: _accentColor.withOpacity(0.7), size: 12)),
                        if (item['isPinned'] == true)
                          Positioned(top: 4, left: 4,
                              child: const Icon(Icons.push_pin, color: NGColors.accentGold, size: 12)),
                        Positioned(
                          left: 4, bottom: 4,
                          child: Row(children: [
                            const Icon(Icons.play_arrow_rounded, color: NGColors.textPrimary, size: 14),
                            const SizedBox(width: 2),
                            Text('${item['viewCount'] ?? 0}',
                                style: const TextStyle(color: NGColors.textPrimary, fontSize: 10, fontWeight: FontWeight.bold)),
                          ]),
                        ),
                      ],
                    ),
                  ...
                  children: [
                    ...
                    if (!_isCurrentUser) ...[
                      ...
                      ElevatedButton(
                        ...
                        child: const Text('Ask a Question', style: TextStyle(color: NGColors.textPrimary)),
                      ),
                    ],
                  ],
                ),
          ...
          if (!_isCurrentUser && idx == 0) {
            return Padding(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('Ask a Question', style: TextStyle(color: NGColors.textPrimary)),
              ),
            );
          }
            ...
            child: Column(
              ...
              children: [
                ...
                if (item['answer'] != null) ...[
                  ...
                  const SizedBox(height: 8),
                  Text(item['answer'], style: const TextStyle(color: NGColors.textSecondary)),
                ],
              ],
            ),
          );
  ...
  Future<void> _askQuestion() async {
    ...
    final result = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: NGColors.surface,
        title: const Text('Ask a Question', style: TextStyle(color: NGColors.textPrimary)),
        content: TextField(
              ...
              child: const Center(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 12),
                    Text('No drafts', style: TextStyle(color: NGColors.textSecondary, fontSize: 14)),
                    SizedBox(height: 4),
                    Text('Unfinished uploads will appear here', style: TextStyle(color: NGColors.textMuted, fontSize: 12)),
                  ],
                ),
              ),
            ),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, idx) {
          ...
          return ListTile(
            ...
            title: Text(draft['title'] ?? 'Untitled Draft', style: const TextStyle(color: NGColors.textPrimary)),
            subtitle: Text('${draft['progress'] ?? 0}% uploaded', style: const TextStyle(color: NGColors.textMuted)),
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                IconButton(icon: const Icon(Icons.upload, color: NGColors.success), onPressed: () {}),
                IconButton(icon: const Icon(Icons.delete_outline, color: NGColors.error), onPressed: () {}),
              ],
            ),
          );
        },
          ...
          child: Column(
            ...
            children: [
              ...
              Text('${(_uploadProgress * 100).toInt()}%',
                  style: const TextStyle(color: NGColors.textSecondary, fontSize: 12)),
            ],
          ),
        ),
      ),
  ...
  Widget _statNode(String val, String title) => Row(
    children: [
      Text(val, style: const TextStyle(color: NGColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 15)),
      const SizedBox(width: 4),
      Text(title, style: const TextStyle(color: NGColors.textSecondary, fontSize: 13)),
    ],
  );
